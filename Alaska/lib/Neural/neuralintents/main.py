...
class myCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        self.epoch_list.append(int(epoch))
        self.accuracy_list.append(logs["accuracy"])
        
    def safe_lists(self):
        file=open('dev-files/NeuralLog/Epoch.txt','a')
        for element in self.epoch_list:
            file.write(str(element))
            file.write('\n')
        file.close()
        file=open('dev-files/NeuralLog/Accuracy.txt','a')
...
class GenericAssistant(IAssistant):
    ...
    def __init__(self, intents, intent_methods={}, model_name="assistant_model"):
        ...
        self.model_path = f"Alaska/lib/Neural/{model_name}/"
    ...
    def train_model(self):
        ...
        self.hist = self.model.fit(np.array(train_x), np.array(train_y), epochs=2000, batch_size=48, verbose=2, use_multiprocessing=True, callbacks=[callbacks])
        callbacks.safe_lists()

    def save_model(self, model_name=None):
        if model_name is None:
            self.model.save(f"{self.model_path}{self.model_name}.h5", self.hist)
            pickle.dump(self.words, open(f'{self.model_path}{self.model_name}_words.pkl', 'wb'))
            pickle.dump(self.classes, open(f'{self.model_path}{self.model_name}_classes.pkl', 'wb'))
        else:
    ...
    def load_model(self, model_name=None):
        if model_name is None:
            self.words = pickle.load(open(f'{self.model_path}{self.model_name}_words.pkl', 'rb'))
            self.classes = pickle.load(open(f'{self.model_path}{self.model_name}_classes.pkl', 'rb'))
            self.model = load_model(f'{self.model_path}{self.model_name}.h5')
        else:
