            ...
            if isfile(file_path):
                with open(file_path, encoding='utf-8') as file:
                    ...
                    if country_info.get('name', None):
                        ...
                        # Update country name if it is one of translations.
                        try:
                            if self.__country_name in map(lambda an: an.lower(), country_info.get('translations', {}).values()):
                                self.__country_name = country_info['name'].lower()
                        except AttributeError:
                            print("Translation Element is None")
    ...
    def timezones(self):
        ...
        if self.__country_name:
            try:
                ...
                return "ERROR: Entered Country not in Database!"
